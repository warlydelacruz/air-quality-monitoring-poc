...
const WeatherCard = ({
  ...
  const fetchAllDataList = useCallback(async () => {
    ...
    const filterCO2 = data.data.filter((item) => item.code === "co2_value");
    const co2Index = Math.floor(Math.random() * filterCO2.length);
    const pm2Index = Math.floor(Math.random() * data.data.length);

    setCurrentCO2(filterCO2[co2Index].value);
    setCurrentPM2(data.data[pm2Index].value);

    setCO2List(filterCO2);
  ...
  const fetchCsv = async () => {
    const response = await fetch(`src/assets/new_file_cbsi_office_sensor.csv`);
